...
class PatchTrainer(object):
    def __init__(self, mode):
        ...
        self.nps_calculator_local = NPSCalculator(self.config.printfile, int(self.config.patch_size/2))
        self.nps_calculator_global = NPSCalculator(self.config.printfile, self.config.patch_size)
        self.css_calculator = CSSCalculator(sample_img=self.read_image(self.config.sampleimgfile))
    ...
    def train(self):
        ...
        ep_tv_loss_l2 = 1
        ep_tv_loss_l3 = 1
        local_det_loss_0 = torch.tensor(0)
        local_nps_loss_0 = torch.tensor(0)
        local_tv_loss_0  = torch.tensor(0)
        local_det_loss_1 = torch.tensor(0)
        local_nps_loss_1 = torch.tensor(0)
        local_tv_loss_1  = torch.tensor(0)
        local_det_loss_2 = torch.tensor(0)
        local_nps_loss_2 = torch.tensor(0)
        local_tv_loss_2  = torch.tensor(0)
        local_det_loss_3 = torch.tensor(0)
        local_nps_loss_3 = torch.tensor(0)
        local_tv_loss_3  = torch.tensor(0)
        global_det_loss = torch.tensor(0)
        global_nps_loss = torch.tensor(0)
        global_tv_loss  =torch.tensor(0)
        et0 = time.time()
        for epoch in range(start_epoch, n_epochs+1):
            ...
            for i_batch, (img_batch, lab_batch) in tqdm(enumerate(train_loader), desc=f'Running epoch {epoch}',
                ...
                with autograd.detect_anomaly():
                    ...
                    if(enable_loss_css):
                        if not(index_local <= -1): # local
                            loss = det_loss + nps_loss + css_loss + torch.max(tv_loss, torch.tensor(0.1))
                        else:
                            loss = det_loss + nps_loss + torch.max(tv_loss, torch.tensor(0.1))
                    else:
                        loss = det_loss + nps_loss + torch.max(tv_loss, torch.tensor(0.1))
                    # loss = det_loss + css_loss + torch.max(tv_loss, torch.tensor(0.1).to(device))
