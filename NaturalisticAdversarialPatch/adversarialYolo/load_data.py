...
class MaxProbExtractor(nn.Module):
    ...
    def forward(self, YOLOoutput):
        ...
        # thre = 0.4
        # zeros = torch.zeros(output_objectness.size()).float().to(device)
        # ones = torch.ones(output_objectness.size()).float().to(device)
...
class PatchSimpleTransformer(nn.Module):
    ...
    def deg_to_rad(self, deg):
        return torch.tensor(deg * pi / 180.0).float().to(device)

    def rad_to_deg(self, rad):
        return torch.tensor(rad * 180.0 / pi).float().to(device)

    def get_warpR(self, anglex, angley, anglez,fov,w,h):
        fov = torch.tensor(fov).float().to(device)
    ...
    def warping(self, input_tensor_img, wrinkle_p = 15):
        ...
        vgrid = perturbed_mesh_2.unsqueeze(0)
        vgrid[:,0,:,:] = 2.0*vgrid[:,0,:,:]/(W-1)-1.0 #max(W-1,1)
        vgrid[:,1,:,:] = 2.0*vgrid[:,1,:,:]/(H-1)-1.0 #max(H-1,1)
        vgrid = vgrid.permute(0,2,3,1)
        input_tensor_img_b = input_tensor_img.unsqueeze(0)
    ...
    def forward(self, adv_patch, lab_batch, img_size, patch_mask=[], do_rotate=True, rand_loc=True, with_black_trans=False, scale_rate=0.2, with_crease=False, with_projection=False, with_rectOccluding=False):
        ...
        contrast = torch.FloatTensor(batch_size).uniform_(self.min_contrast, self.max_contrast).to(device)

        contrast = contrast.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        contrast = contrast.expand(-1, -1, adv_batch.size(-3), adv_batch.size(-2), adv_batch.size(-1))
        contrast = contrast.to(self.device)
        # print("contrast size : "+str(contrast.size()))  ##  contrast size : torch.Size([8, 14, 3, 300, 300])


        # Create random brightness tensor
        brightness = torch.FloatTensor(batch_size).uniform_(self.min_brightness, self.max_brightness).to(device)
        ...
        def resize_rotate(adv_batch):
            ...
            msk_batch = torch.FloatTensor(cls_mask.size()).fill_(1).to(device) - cls_mask  # torch.Size([8, 14, 3, 300, 300])
            ...
            if do_rotate:
                angle = torch.FloatTensor(anglesize).uniform_(self.minangle, self.maxangle).to(device)  # torch.Size([112])
            else: 
                angle = torch.FloatTensor(anglesize).fill_(0)

            # Resizes and rotates
            current_patch_size = adv_patch.size(-1)
            lab_batch_scaled = torch.FloatTensor(lab_batch.size()).fill_(0).to(device)  # torch.Size([8, 14, 5])
            ...
            theta = torch.FloatTensor(anglesize, 2, 3).fill_(0).to(device)  # torch.Size([112, 2, 3])
            theta[:, 0, 0] = cos/scale
            theta[:, 0, 1] = sin/scale
...
class PatchTransformer(nn.Module):
    ...
    def __init__(self, device=torch.device("cpu")):
        ...
        # Create the kernel tensor and move it to the specified device
        kernel = torch.FloatTensor([[0.003765, 0.015019, 0.023792, 0.015019, 0.003765],
                                     [0.015019, 0.059912, 0.094907, 0.059912, 0.015019],
                                     [0.023792, 0.094907, 0.150342, 0.094907, 0.023792],
                                     [0.015019, 0.059912, 0.094907, 0.059912, 0.015019],
                                     [0.003765, 0.015019, 0.023792, 0.015019, 0.003765]])
        self.kernel = kernel.unsqueeze(0).unsqueeze(0).expand(3, 3, -1, -1).to(self.device)
    ...
    def forward(self, adv_patch, lab_batch, img_size, patch_mask=[], by_rectangle=False, do_rotate=True, rand_loc=True, with_black_trans=False, scale_rate=0.2, with_crease=False, with_projection=False, with_rectOccluding=False, enable_empty_patch=False, enable_no_random=False, enable_blurred=True):
        ...
        ## get y gray
        # adv_patch_yuv = Colorspace("rgb", "yuv")(adv_patch).to(device)
        # y = adv_patch_yuv[0].unsqueeze(0)
        # adv_patch_new_y_gray = torch.cat((y,y,y), 0).to(device)
        ## get   gray
        ...
        # adv_batch_new_y_gray = torch.cat((y,y,y), 2).to(device)
        ...
        #         adv_batch_unit = adv_batch[:,:,:, x_s:x_e, y_s:y_e].to(device)
        #         adv_batch_zeroes = torch.zeros(adv_batch.size()).to(device)
        ...
        def resize_rotate(adv_batch, by_rectangle=False):
            ...
            anglesize = (lab_batch.size(0) * lab_batch.size(1)) # 8*14 = 112
            if do_rotate:
                angle = torch.cFloatTensor(anglesize).uniform_(self.minangle, self.maxangle).to(device)  # torch.Size([112])
            else: 
...
if __name__ == '__main__':
    ...
    prob_extractor = MaxProbExtractor(0, 80)
