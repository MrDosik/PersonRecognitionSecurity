    ...
    if(model_name == "yolov3" or model_name == "yolov4"):
        for b in bbox:
            if (int(b[-1]) == int(cls_id_attacked)):
                label = np.array([
                    b[-1].detach().cpu().numpy(),
                    (b[0].detach().cpu().numpy() + b[2].detach().cpu().numpy()) / 2.0,
                    (b[1].detach().cpu().numpy() + b[3].detach().cpu().numpy()) / 2.0,
                    (b[2].detach().cpu().numpy() - b[0].detach().cpu().numpy()),
                    (b[3].detach().cpu().numpy() - b[1].detach().cpu().numpy()),
                    b[4].detach().cpu().numpy()
                ], dtype=np.float32)
                labels.append(label)
                b = b.clone()  # Ensure b is a new tensor (not a view)
                b[:-3] = b[:-3] * img_side
                label_rescale = np.array([b[-1].detach().cpu().numpy(),
                                          b[-2].detach().cpu().numpy(),
                                          b[0].detach().cpu().numpy(),
                                          b[1].detach().cpu().numpy(),
                                          b[2].detach().cpu().numpy(),
                                          b[3].detach().cpu().numpy()], dtype=np.float32)
