...
def gen_mask_set(args,ds_config):
    ...
    for x in idx_list1:
        for y in idx_list2:
            mask = torch.ones([1,1,IMG_SIZE[0],IMG_SIZE[1]],dtype=bool)
        ...
        if len(pred)==1: # unanimous agreement in the first-round masking
            ...
            for i in disagreer_pred_mask_idx:
                ...
                for mask2 in mask_list:
                    ...
                    masked_output = model(torch.where(torch.logical_and(mask,mask2),data[j],torch.tensor(0.)))
    ...
    for j in range(num_img):
        ...
        if len(pred)==1: # unanimous agreement in the first-round masking
            ...
            while len(np.unique(pred_one_mask[~used_flg]))>1:
                ...
                masked_output = model(torch.where(torch.logical_and(candidate_mask,challenger_mask),data[j],torch.tensor(0.)))
                _, masked_pred = masked_output.max(1)
                masked_pred = masked_pred.item()
